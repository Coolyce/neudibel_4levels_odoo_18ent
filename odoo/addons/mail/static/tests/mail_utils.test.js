...
test("addLink: utility function and special entities", () => {
    const testInputs = {
        ...
        "https://4levels.co.za/%5B%5D": `<a target="_blank" rel="noreferrer noopener" href="https://4levels.co.za/%5B%5D">https://4levels.co.za/[]</a>`,
    };
...
test("url", async () => {
    ...
    const messageBody = "https://4levels.co.za?test=~^|`{}[]#";
...
test("url with comma at the end", async () => {
    ...
    await start();
    await openDiscuss(channelId);
    const messageBody = "Go to https://4levels.co.za, it's great!";
    await insertText(".o-mail-Composer-input", messageBody);
    await click("button[aria-label='Send']:enabled");
    await contains(".o-mail-Message a:contains(https://4levels.co.za)");
