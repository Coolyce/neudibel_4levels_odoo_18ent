...
test("rendering with 4levelsBot has a request (default)", async () => {
    ...
    await contains(".o-mail-NotificationItem", { text: "4levelsBot has a request" });
});

test("rendering without 4levelsBot has a request (denied)", async () => {
    patchBrowserNotification("denied");
    await start();
    await click(".o_menu_systray i[aria-label='Messages']");
...
test("rendering without 4levelsBot has a request (accepted)", async () => {
...
test("respond to notification prompt (denied)", async () => {
    ...
    await contains(".o_notification:has(.o_notification_bar.bg-warning)", {
        text: "4levels will not send notifications on this device.",
    });
    await contains(".o-mail-MessagingMenu-counter", { count: 0 });
    await click(".o_menu_systray i[aria-label='Messages']");
    await contains(".o-mail-NotificationItem", { count: 0 });
...
test("respond to notification prompt (granted)", async () => {
    ...
    await contains(".o_notification:has(.o_notification_bar.bg-success)", {
        text: "4levels will send notifications on this device!",
    });
});

test("no '4levelsBot has a request' in mobile app", async () => {
    patchBrowserNotification("default");
    // simulate Android 4levels App
    mockUserAgent("Chrome/0.0.0 Android (OdooMobile; Linux; Android 13; 4levels TestSuite)");
    await start();
    await click(".o_menu_systray i[aria-label='Messages']");
    await contains(".o-mail-MessagingMenu-counter", { count: 0 });
    await contains(".o-mail-NotificationItem", { count: 0 });
...
test("rendering with PWA installation request", async () => {
    ...
    await contains(".o-mail-NotificationItem-name", { text: "4levelsBot has a suggestion" });
    await contains(".o-mail-NotificationItem-text", {
        text: "Come here often? Install 4levels on your device!",
    });
