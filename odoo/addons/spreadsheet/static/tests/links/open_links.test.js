...
test("click a web link", async () => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: { content: "[Odoo](https://4levels.co.za)" } },
            },
        ],
    };
    const model = new Model(data, { custom: { env } });
    const cell = getEvaluatedCell(model, "A1");
    expect(urlRepresentation(cell.link, model.getters)).toBe("https://4levels.co.za");
    openLink(cell.link, env);
    expect.verifySteps(["https://4levels.co.za"]);
});
