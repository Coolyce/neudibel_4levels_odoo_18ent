# Part of 4levels. See LICENSE file for full copyright and licensing details.
r"""\
4levels HTTP layer / WSGI application
...
Application.__call__
  ...
  request and itself in an 4levels http request. The 4levels http request is
...
4levels URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://4levels.co.za/documentation/master/developer/reference/addons/http.html#csrf
for more details.

* if this endpoint is accessed through 4levels via py-QWeb form, embed a CSRF
...
def db_filter(dbs, host=None):
    ...
    if config['db_name']:
        # In case --db-filter is not provided and --database is passed, 4levels will
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    ...
    def build_controllers():
        ...
        installed modules). Modules in this context are 4levels addons.
        """
...
class JsonRPCDispatcher(Dispatcher):
    ...
    def handle_error(self, exc: Exception) -> collections.abc.Callable:
        ...
        if isinstance(exc, NotFound):
            ...
            error['code'] = 100
            error['message'] = "4levels Session Expired"
